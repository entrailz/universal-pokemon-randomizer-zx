...
public class CliRandomizer {
    ...
    public static int invoke(String[] args) {
        ...
        String generateSettingsInputString = null;
        String inputSettingsInputString = null;

        List<String> allowedFlags = Arrays.asList("-i", "-o", "-s", "-d", "-u", "-l", "-g", "-ss", "--help");
        for (int i = 0; i < args.length; i++) {
            if (allowedFlags.contains(args[i])) {
                switch(args[i]) {
                    case "-g":
                        generateSettings = true;
                        generateSettingsInputString = args[i + 1];
                        break;
                    case "-ss":
                        inputSettingsInputString = args[i + 1];
                    case "-i":
        ...
        if (inputSettingsInputString != null)
        {
            Settings settings = getSettingsFromString(inputSettingsInputString);
    ...
    private static Settings getSettingsFromString(String settingsString)
    {
        ...
        try
        {
            ...
            for (RomHandler.Factory rhf : checkHandlers) {
                ...
                if (rhf.isLoadable(chosenFile.getAbsolutePath()))
                {
                    ...
                    if (isNumeric(splitSettings[2].substring(0,1)))
                    {
                        splitSettings[2] = splitSettings[2].substring(3);
                    }
                    Settings settings = Settings.fromString(splitSettings[2]);
